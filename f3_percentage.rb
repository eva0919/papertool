...
# model : using self data and direct competitor
require 'date'
maxYear = 2014
filenameAppend = "All_action_Round2_"
output_filenameAppend ="RowData_weka_DC_logic_round2_representIsPercentage_" 
				...
				(0..3).each do |ind|
					if ind < 3
						...
						hash[timeZone[ind]].each_index do |value|
							...
							if totalValue_eachCompanyHash[competitorTable[companyName.to_sym].to_sym][timeZone[ind]].to_i > 0
								competitorValue = competitorValue.to_f / totalValue_eachCompanyHash[competitorTable[companyName.to_sym].to_sym][timeZone[ind]].to_i
							else 
